...
def getMachine(mtype):

    if mtype == 'SVM Poly3':
        return  svm.SVC(gamma=0.1,kernel='poly',degree=3)
    elif mtype == 'Gaussian':
        return GaussianNB()
    elif mtype == 'Logistic Regression':
        return sk.LogisticRegression()
    elif mtype =='SVM Rbf':
        return svm.SVC(gamma=0.01,kernel='rbf')
    else:
        return None

def Learn(data,target,mtype,pct,mname):
    X_train, X_test, y_train, y_test = cross_validation.train_test_split(data, target, test_size=pct/100.0)
    rfc = getMachine(mtype)
    print mtype, "fitting"
    rfc.fit(X_train, y_train)
    print mtype, "predicting"
    joblib.dump(rfc, PREFIX + mname+'.mod')
    xpred = rfc.predict(data)
    print "done"
    return xpred


def Run(data,mname):
    estimator = joblib.load(PREFIX + "%s.mod"%mname)
    print mname, 'successfully loaded'
    print "predicting:"
    xpred = estimator.predict(data)
    print "done"
    return xpred
