...
def getConf(request,name):
    ...
    rawg =  persist.load_json(name + '.raw')
    expg =  persist.load_json(name + '.exp')
    forg =  persist.load_json(name + '.run')
    msg = { 'message': cont,'raw':rawg,'exp':expg,'run':forg}
...
def getGraph(request,lists,graphtype,dtfilter,req,expression,mtype,maw,pct,mname,learn):
    ...
    js = json.dumps({'arg': arg,'message': series})
    persist.save_json(mname, {'arg': arg,'message': series})
    return js
